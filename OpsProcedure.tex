...
V17 is a metal valve, 
and we close V17 to prevent cold Ar gas from entering the gas heater.
...
\item Cool down the LAr purifier by filling it with LAr from V3 and venting the gas Ar from V7.  
...
\item Load $\sim$55~L of nitrogen in the thermosyphon line 11 (TSL11).
    \begin{itemize}
        ...
        \item Use the ``Thermosyphon'' page from the DUNE detector control system (Ignition).
        \item Use the ``Purge'' bottom to purge the thermosyphoton line.
        ...
        amount accordingly (usually adding or removing 1-3~L of nitrogen).
    \end{itemize}
\item LArPix DAQ tutorial can be found at \url{https://github.com/SLACube/slacube-daq-tutorial}.
    \begin{itemize}
        \item DO NOT run DAQ when the system is in vacuum.
        \item Configure the hydra network, take pedestal runs and find bad channels at room temperature.
              Then do threshold and selftrigger tests at room temperature as well.
        \item Monitor pedestal during LAr filling (after the vessel is at $\ge$ atmospheric pressure).
        \item After LAr is filled, set threshold and test the trigger rate with HV = 0V.
        \item After HV is ramped up, try a few selftrigger tests.  Manually increase the trigger threshold
              if the trigger rate is too high ($>2$Hz per channel).
    \end{itemize}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[angle=90,origin=c,height=7.2in]{fig/PIDv8.4.pdf}
\caption{P\&ID}
\end{center}
\end{figure}
%-------------------------------------------------------------------
...
\begin{figure}[htb]
\begin{center}
\includegraphics[angle=90,origin=c,height=7.2in]{fig/PurifierCooling_PIDv8.4.pdf}
...
\begin{figure}[htb]
\begin{center}
\includegraphics[angle=90,origin=c,height=7.2in]{fig/LArFilling_PIDv8.4.pdf}
...
\myCheckBox{LArPix tests in the room temperature at atmosphere: Configure the hydra network,
take pedestal runs, find bad channels, check the threshold and test selftrigger rates.} 
& LArPix tutorial at 
...
\myCheckBox{V30 and the valve on the Hicube pump open} & The Hicube pump is located behind the computer monitor.  
V30 is connected on the thermosyphon evaporator, and is not shown in the current version of P\&ID \\
...
\myCheckBox{The second operator opens V28 (LAr dewar) gradually, and the first operator opens V13 according 
to the pressure in the vessel (PG5 or PT1)} & \\
...
\myCheckBox{Thermosyphone load: Fill with 10~L at 10~slpm, and the pressure is less than 5~bar 
(better less than 3~bar)} & 
Use the ``Thermosyphon page''.  Fill the numbers 10~L (large box) at 10~slpm, 
and click \texttt{Add N$_2$}.
...
\myCheckBox{Equilibrium reached and {\color{orange}$\sim$20~psig (TBC)} at PG3 (pressure 
gauge on top of the LAr filter)} & \\
...
% Align the connector, and if it is not well aligned, loosen the fitting at V1.  
% If you loosen the fitting at V1, don't forget to change the gasket \\
\myCheckBox{V1, V2 opened} & Prepare to purge the air \\
\myCheckBox{V28 (LAr dewar) open} & Purge the air in the tube \\
\myCheckBox{When seeing LAr from V2, V28 (LAr dewar), V2 closed} & \\
\myCheckBox{V3 open} & \\
\myCheckBox{Double check V9 opened} & \\
\myCheckBox{One operator ready for adjusting V13 all the time according to the pressure in SLArchetto.} &
We want to keep the pressure at about 2~psig at PG5 (16.6~psia at PT1) and not to exceed 
4~psig at PG5 (18.6~psia at PT1) all the time.
We also don’t want the vessel pressure to go below 0~psig at PG5 (14.6~psia at PT1), 
in which condition the air would come in and contaminate the LAr purity. \\
\myCheckBox{The second operator opens V28 (LAr dewar) gradually, and the first operator
open/adjust V13 according to the pressure in the vessel (PG5 or PT1)} & \\
...
Liquid level at 65~cm (SLArchetto page in Ignition)} & 
...
\myCheckBox{V7 open} & Vent LAr filter to the venting line. \\
...
\myCheckBox{Set the threshold of LArPix channels with HV off and test the selftrigger rate} & 
...
\myCheckBox{V7 closed when LAr filter venting completed} & \\
\myCheckBox{All valves closed} & 
...
at $\sim$9000 -- 10000~nA (TBC)} & Check \texttt{Cathode Voltage} and \texttt{PicoAm Current} in 
...
\myCheckBox{Enable the warning, alert, and alarm for the current, and the alarms on updates 
of the current} & 
Click the alarm button.  Warning range: -20,000 -- 0~nA; Alert range: -25,000 -- 0~nA; 
Alarm range: -30,000 -- 0~nA (TBC) \\
